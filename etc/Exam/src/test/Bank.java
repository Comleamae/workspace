...
public class Bank {
    ...
    String showClInfo(int clientNumber, String name) {
        int index = 0;

        for (int i = 0; i < clientList.size(); i++) {
            if (clientNumber == clientList.get(i).getClientNumber() && name.equals(clientList.get(i).getName())) {
                    index = i ;
                }
        }
        if(clientNumber == clientList.get(index).getClientNumber()){
            //System.out.println(clientList.get(index).toString());
            return clientList.get(index).toString();
        }
        else{
            return null;
        }
